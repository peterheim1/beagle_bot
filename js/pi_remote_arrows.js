...
function pubHeadCmd() {
    if (centerServos) {
	cameraTiltMarker.setFill(green);
	cameraPanMarker.setFill(green);
	videoMarkerLayer.draw();
        servosRelaxed = false;
	centerServos = false;
	...
	cameraTiltMarker.setFill(yellow);
	cameraPanMarker.setFill(yellow);
	videoMarkerLayer.draw();
        servosRelaxed = false;
...
function updateCmdVelMarker() {
    ...
    var y = videoStageHeight - vx * 500 * videoScale;
...
function doClickNav(x, y) {
    ...
    var rotate = headPanPosition -  fovWidthRadians * ((x / videoScale - videoWidth / 2) / videoWidth);
    log(rotate);
